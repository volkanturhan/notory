# Builds both shareable notory packages and gathers them under dist/release:
#
#   notory.exe       self-contained (~68 MB) — runs without installing .NET
#   notory-lite.exe  framework-dependent (~0.4 MB) — needs the .NET 8 Desktop
...
Copy-Item (Join-Path $selfContainedDir 'notory.exe') (Join-Path $releaseDir 'notory.exe') -Force
Copy-Item (Join-Path $liteDir 'notory.exe') (Join-Path $releaseDir 'notory-lite.exe') -Force
