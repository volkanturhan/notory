# Builds a complete notory release — the portable self-contained exe and the
# Windows installer — both named with the version read from notory.csproj.
#
#   .\tools\release.ps1                   -> artifacts land in dist\release
#   .\tools\release.ps1 -OutDir <folder>  -> also copied into <folder>
#
# The -OutDir hook lets the outer workspace collect the build into builds\notory
...
Write-Output "Building notory v$version"
...
$portable = Join-Path $releaseDir "notory-v$version.exe"
$setup    = Join-Path $releaseDir "notory-setup-v$version.exe"
Copy-Item (Join-Path $selfContainedDir 'notory.exe') $portable -Force
Copy-Item (Join-Path $root "dist\installer\notory-setup-v$version.exe") $setup -Force
